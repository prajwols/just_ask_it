...
const Services = () => {
  ...
  const ServiceCard = ({
    ...
    item: (typeof services)[number] | (typeof software)[number];
  }) => (
    <div className="group relative p-6 rounded-2xl bg-black/40 hover:bg-black/60 transition-all duration-500 hover:scale-105 cursor-pointer border border-white/10 overflow-hidden min-h-[450px]">
      ...
      <div className="relative z-10">
        <div className="inline-flex p-4 rounded-xl bg-slate-950 mb-6 group-hover:scale-110 transition-transform duration-300 dark:bg-gradient-to-r dark:from-[#0033CC] dark:to-[#8F94FF]">
          <item.icon className="w-8 h-8 text-white" />
        </div>

        <h3 className="mb-4 font-title text-xl font-bold text-white transition-colors duration-300 group-hover:text-[#8F94FF]">
          {item.title}
        </h3>

        <p className="mb-6 font-content text-sm leading-relaxed text-white/80">
          {item.description}
        </p>

        <button
          onClick={() => scrollToSection("booking-section")}
          className="group/link flex items-center border-b border-white/30 pb-1 font-content text-sm font-medium text-white transition-colors duration-300 hover:border-[#8F94FF]/50 hover:text-[#8F94FF]"
        >
          {item.cta}
          <item.ctaIcon className="ml-2 w-4 h-4 transition-transform duration-300 group-hover/link:translate-x-1" />
        </button>
      </div>
    </div>
  );
